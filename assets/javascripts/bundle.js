...
/***/ function(module, exports) {
	...
	Elm.Pipeliner = Elm.Pipeliner || {};
	Elm.Pipeliner.make = function (_elm) {
	   "use strict";
	   _elm.Pipeliner = _elm.Pipeliner || {};
	   if (_elm.Pipeliner.values) return _elm.Pipeliner.values;
	   var _U = Elm.Native.Utils.make(_elm),
	   $Array = Elm.Array.make(_elm),
	   $Basics = Elm.Basics.make(_elm),
	   $Debug = Elm.Debug.make(_elm),
	   ...
	   var Sort = {ctor: "Sort"};
	   var Remove = {ctor: "Remove"};
	   var CommitCode = function (a) {    return {ctor: "CommitCode",_0: a};};
	   var Add = {ctor: "Add"};
	   var NoOp = {ctor: "NoOp"};
	   var inbox = $Signal.mailbox(NoOp);
	   var statusLog = A2($Html.div,
	   _U.list([$Html$Attributes.$class("ui info message")]),
	   _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("header")]),_U.list([$Html.text("Info Log Output")]))
	           ,A2($Html.ul,
	           _U.list([$Html$Attributes.$class("list")]),
	           _U.list([A2($Html.li,_U.list([]),_U.list([$Html.text("it\'s good")])),A2($Html.li,_U.list([]),_U.list([$Html.text("it\'s better")]))]))]));
	   var sideBar = function (pipelineRunning) {
	      var buttonClass = function () {
	         var _p0 = pipelineRunning;
	         if (_p0 === true) {
	               return "ui button loading";
	            } else {
	               return "ui green button inverted";
	            }
	      }();
	      return A2($Html.div,
	      _U.list([$Html$Attributes.$class("five wide column")]),
	      _U.list([A2($Html.div,
	      _U.list([$Html$Attributes.$class("pipeline-sidebar")]),
	      _U.list([A2($Html.div,_U.list([$Html$Attributes.id("editor"),$Html$Attributes.$class("pipeline-sidebar__editor")]),_U.list([]))
	              ,A2($Html.button,
	              _U.list([$Html$Attributes.id("commit"),$Html$Attributes.$class(buttonClass),$Html$Attributes.disabled(pipelineRunning)]),
	              _U.list([$Html.text("Commit")]))]))]));
	   };
	   var mapStatusToIcon = F2(function (status,icon) {
	      var _p1 = status;
	      switch (_p1.ctor)
	      {case "Running": return "setting loading icon";
	         case "Failed": return "red warning icon";
	         default: return "";}
	   });
	   var mapStatusToCss = function (status) {
	      var _p2 = status;
	      switch (_p2.ctor)
	   ...
	   var stepItem = F2(function (address,entry) {
	      var status = A2($Basics._op["++"],mapStatusToCss(entry.status)," card");
	      var icon = A2(mapStatusToIcon,entry.status,entry.icon);
	      return A2($Html.a,
	      _U.list([$Html$Attributes.$class(status)]),
	      _U.list([A2($Html.div,
	      _U.list([$Html$Attributes.$class("content")]),
	      _U.list([A2($Html.div,
	              _U.list([$Html$Attributes.$class("header"),$Html$Attributes.contenteditable(true)]),
	              _U.list([$Html.text(A2($Basics._op["++"],$Basics.toString(entry.id),A2($Basics._op["++"]," ",entry.title)))
	                      ,A2($Html.i,_U.list([$Html$Attributes.$class(icon)]),_U.list([]))]))
	              ,A2($Html.div,
	              _U.list([$Html$Attributes.$class("meta")]),
	              _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("right floated time")]),_U.list([$Html.text("Last build: 2 days ago")]))
	                      ,A2($Html.span,_U.list([$Html$Attributes.$class("category")]),_U.list([$Html.text("Waiting")]))]))
	              ,A2($Html.div,
	              _U.list([$Html$Attributes.$class("description"),$Html$Attributes.contenteditable(true)]),
	              _U.list([A2($Html.p,_U.list([]),_U.list([$Html.text(entry.description)]))
	                      ,A2($Html.a,
	                      _U.list([$Html$Attributes.$class("ui right corner label")]),
	                      _U.list([A2($Html.i,_U.list([$Html$Attributes.$class("info icon")]),_U.list([]))]))]))
	              ,A2($Html.div,
	              _U.list([$Html$Attributes.$class("extra content"),$Html$Attributes.contenteditable(true)]),
	              _U.list([$Html.text("Add description")
	                      ,A2($Html.div,_U.list([$Html$Attributes.$class("right floated author")]),_U.list([$Html.text("Author")]))]))]))]));
	   });
	   var stepList = F2(function (address,steps) {
	      var stepItems = A2($List.map,stepItem(address),steps);
	      return A2($Html.div,
	      _U.list([$Html$Attributes.id("builds"),$Html$Attributes.$class("eleven wide column")]),
	      _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("ui three cards")]),stepItems)
	              ,$Html.text(" ")
	              ,A2($Html.button,
	              _U.list([$Html$Attributes.$class("circular ui icon button"),A2($Html$Events.onClick,address,Add)]),
	              _U.list([A2($Html.i,_U.list([$Html$Attributes.$class("icon plus")]),_U.list([]))]))
	              ,A2($Html.button,
	              _U.list([$Html$Attributes.$class("circular ui icon button"),A2($Html$Events.onClick,address,Remove)]),
	              _U.list([A2($Html.i,_U.list([$Html$Attributes.$class("icon minus")]),_U.list([]))]))]));
	   });
	   var Model = F5(function (a,b,c,d,e) {    return {title: a,description: b,code: c,annotations: d,steps: e};});
	   var step = F5(function (title,description,id,icon,status) {    return {code: "",status: status,title: title,description: description,icon: icon,id: id};});
	   var Step = F6(function (a,b,c,d,e,f) {    return {code: a,status: b,title: c,description: d,icon: e,id: f};});
	   var Waiting = {ctor: "Waiting"};
	   var initialModel = {title: "Hello Pipelines!"
	                      ,description: "Example description"
	                      ,code: ""
	                      ,annotations: _U.list([])
	                      ,steps: _U.list([A5(step,"Build","Compile & Unit tests",1,"",Waiting)
	                                      ,A5(step,"Deployment DEV","via SSH to DEV",2,"send",Waiting)
	                                      ,A5(step,"Testing DEV","Running automated tests",3,"unhide",Waiting)
	                                      ,A5(step,"Deployment SIT","Running automated User Acceptance Tests",4,"warning",Waiting)])};
	   var Success = {ctor: "Success"};
	   var Failed = {ctor: "Failed"};
	   var Running = {ctor: "Running"};
	   var startPipeline = F2(function (code,steps) {
	      var _p3 = steps;
	      if (_p3.ctor === "[]") {
	            return _U.list([]);
	         } else {
	            var _p4 = _p3._0;
	            return A2($List._op["::"],{code: code,status: Running,title: _p4.title,description: _p4.description,icon: _p4.icon,id: _p4.id},_p3._1);
	         }
	   });
	   var view = F2(function (address,model) {
	      ...
	      _U.list([]),
	      _U.list([A2($Html.h1,
	              _U.list([$Html$Attributes.$class("ui header")]),
	              _U.list([A2($Html.i,_U.list([$Html$Attributes.$class("settings icon")]),_U.list([]))
	                      ,A2($Html.div,
	                      _U.list([$Html$Attributes.$class("content"),$Html$Attributes.contenteditable(true)]),
	                      _U.list([$Html.text(model.title)
	                              ,A2($Html.div,
	                              _U.list([$Html$Attributes.$class("sub header"),$Html$Attributes.contenteditable(true)]),
	                              _U.list([$Html.text(model.description)]))]))]))
	              ,A2($Html.hr,_U.list([]),_U.list([]))
	              ,A2($Html.div,
	              _U.list([$Html$Attributes.$class("ui two column divided grid")]),
	              _U.list([A2($Html.div,
	              _U.list([$Html$Attributes.$class("row")]),
	              _U.list([sideBar(A2($List.any,function (s) {    return _U.eq(s.status,Running);},model.steps)),A2(stepList,address,model.steps)]))]))
	              ,A2($Html.div,
	              _U.list([$Html$Attributes.$class("ui top attached tabular menu")]),
	              _U.list([A2($Html.a,
	                      _U.list([$Html$Attributes.$class("item"),A2($Html$Attributes.attribute,"data-tab","first")]),
	                      _U.list([$Html.text("System Log")]))
	                      ,A2($Html.a,
	                      _U.list([$Html$Attributes.$class("item"),A2($Html$Attributes.attribute,"data-tab","second")]),
	                      _U.list([$Html.text("Build log")]))
	                      ,A2($Html.a,
	                      _U.list([$Html$Attributes.$class("item active"),A2($Html$Attributes.attribute,"data-tab","third")]),
	                      _U.list([$Html.text("Site Preview")]))]))
	              ,A2($Html.div,
	              _U.list([$Html$Attributes.$class("ui bottom attached tab segment"),A2($Html$Attributes.attribute,"data-tab","first")]),
	              _U.list([A2($Html.pre,
	              _U.list([]),
	              _U.list([$Html.text("[INFO] Pipeline up and running.")
	                      ,A2($Html.br,_U.list([]),_U.list([]))
	                      ,$Html.text("[INFO] Jobs waiting for action.")
	                      ,A2($Html.br,_U.list([]),_U.list([]))
	                      ,$Html.text("[INFO] Code commited to Build.")]))]))
	              ,A2($Html.div,
	              _U.list([$Html$Attributes.$class("ui bottom attached tab segment"),A2($Html$Attributes.attribute,"data-tab","second")]),
	              _U.list([A2($Html.pre,_U.list([]),_U.list([$Html.text("[INFO] Waiting.")]))]))
	              ,A2($Html.div,
	              _U.list([$Html$Attributes.$class("ui bottom attached tab segment active"),A2($Html$Attributes.attribute,"data-tab","third")]),
	              _U.list([A2($Html.iframe,
	              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "backgroundColor",_1: "red"}
	                                                      ,{ctor: "_Tuple2",_0: "height",_1: "190px"}
	                                                      ,{ctor: "_Tuple2",_0: "width",_1: "100%"}]))
	                      ,$Html$Attributes.srcdoc("<div>Website preview inside iframe</div><h1>No code has been deployed yet. Keep it coming!</h1>")]),
	              _U.list([]))]))]));
	   });
	   var Annotation = F5(function (a,b,c,d,e) {    return {row: a,column: b,errorType: c,raw: d,text: e};});
	   var annotationsDecoder = $Json$Decode.list(A6($Json$Decode.object5,
	   Annotation,
	   A2($Json$Decode._op[":="],"row",$Json$Decode.$int),
	   $Json$Decode.maybe(A2($Json$Decode._op[":="],"column",$Json$Decode.$int)),
	   A2($Json$Decode._op[":="],"errorType",$Json$Decode.string),
	   A2($Json$Decode._op[":="],"raw",$Json$Decode.string),
	   A2($Json$Decode._op[":="],"text",$Json$Decode.string)));
	   var update = F2(function (action,model) {
	      var _p5 = action;
	      switch (_p5.ctor)
	      {case "NoOp": return model;
	         case "Sort": return _U.update(model,{steps: A2($List.sortBy,function (_) {    return _.id;},model.steps)});
	         case "Remove": return _U.update(model,{code: "",steps: $Array.toList(A3($Array.slice,0,-1,$Array.fromList(model.steps)))});
	         case "Add": var currentId = $List.maximum(A2($List.map,function (_) {    return _.id;},model.steps));
	           var newId = A2($Maybe.withDefault,0,currentId) + 1;
	           var entryToAdd = A5(step,A2($Basics._op["++"],"Job",$Basics.toString(newId)),"",newId,"setting",Waiting);
	           return _U.update(model,{code: "",steps: $Array.toList(A2($Array.push,entryToAdd,$Array.fromList(model.steps)))});
	         default: var _p6 = _p5._0._0;
	           return _U.update(model,
	           {code: _p6
	           ,annotations: A2($Result.withDefault,_U.list([]),A2($Json$Decode.decodeValue,annotationsDecoder,_p5._0._1))
	           ,steps: A2(startPipeline,_p6,model.steps)});}
	   });
	   var currentCode = Elm.Native.Port.make(_elm).inboundSignal("currentCode",
	   "( String, Json.Decode.Value )",
	   function (v) {
	      return typeof v === "object" && v instanceof Array ? {ctor: "_Tuple2"
	                                                           ...
	                                                           v[0])
	                                                           ,_1: v[1]} : _U.badPort("an array",v);
	   });
	   var actions = A2($Signal.merge,inbox.signal,A2($Signal.map,CommitCode,currentCode));
	   var model = A3($Signal.foldp,update,initialModel,actions);
	   var main = A2($Signal.map,view(inbox.address),model);
	   return _elm.Pipeliner.values = {_op: _op
	                                  ,Annotation: Annotation
	                                  ,annotationsDecoder: annotationsDecoder
	                                  ,Running: Running
	                                  ,Failed: Failed
	                                  ,Success: Success
	                                  ,Waiting: Waiting
	                                  ...
	                                  ,mapStatusToCss: mapStatusToCss
	                                  ,mapStatusToIcon: mapStatusToIcon
	                                  ,stepItem: stepItem
	                                  ,stepList: stepList
	                                  ,sideBar: sideBar
	                                  ,statusLog: statusLog
